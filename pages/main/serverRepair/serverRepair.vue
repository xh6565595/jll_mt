			...
			<block v-for="(item, index) in list" :key="index">
				<view class="orderItems"  >
					<view class="flex flex_center cm_bdb top">
						<view class="f1 cm_ellipsis">服务时间：{{ item.booking_time }}</view>
						<text class="status ">{{ item.mend_status| mend_statusFilter }}</text>
					</view>
					<view class="content" @tap="_scanCode(item)">
						...
						<view class="flex flex_center listItems " v-if="item.mend_status!=1">
						...
						<view class="flex flex_center listItems "  v-if="item.mend_status!=2">
						...
						<view class="flex flex_center listItems last" v-if="item.mend_status!=1">
					...
					<view class="footer flex flex_center">
						<view class="f1"></view>
						<tuiButton size="mini"   type="primary"  style="margin-right: 10rpx;" v-if="item.mend_status!=1"  @tap="_scanCode(item)">{{item.mend_status==2?'扫描设备':'立即维修'}}</tuiButton>
						<tuiButton size="mini"   type="default" style="margin-right: 10rpx;"  v-if="item.mend_status!=1" @tap="_call(item.booking_mobile)">致电联系人</tuiButton>
...
export default {
	...
	methods: {
		...
		//扫码
		_scanCode(item) {
			let wcode = item.task_code;
			let oid = item.order_code;
			let status = item.mend_status;
			let that = this;
			this.repairParams.order_code = oid;
			// debugger
			if (status == 3 || status == 1) {
				uni.navigateTo({
					url: `/pages/main/repairDetail/repairDetail?code=${oid}&wcode=${wcode}`
				});
			} else if (status == 2) {
				this.$ui.showloading();
				uni.scanCode({
					...
					success: function(res) {
						...
						if (result && s && s[1]) {
							that.repairParams.device_num = s[1];
							that._checkRepair(oid,wcode);
						} else {
							uni.showModal({
								title: '智能厨卫提醒您',
		...
		async _checkRepair(oid,wcode) {
			...
			// }
			let res = await this.$api.CheckMendTask(this.repairParams, false);
			this.$ui.hideloading();

			if (res.Success) {
				// that.install_info = res.Data
				uni.setStorageSync('repairMsg', res.Data);
				uni.navigateTo({
					// url: '/pages/main/repairMsg/repairMsg?wcode=' + wcode,
					url: `/pages/main/repairMsg/repairMsg?device=${that.repairParams.device_num }&wcode=${wcode}`
				});
			} else {
		...
		async _loadData(type) {
			let that = this;
			try {
				let res = await this.$api.GetMendTaskList(this.formParams, false);
				// console.log(res);
				if (type == 'add') {
				...
				if (res.Success) {
					if (type == 'add') {
						...
						that.tabObjectList = [
							{
								name: `待维修(${res.Data.count.await_count})`,
								value: 2
							},
							{
